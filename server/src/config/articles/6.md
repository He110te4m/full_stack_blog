# 一. Hexo 配置公益 404 页面 #
从 [腾讯公益 404 页面](http://www.qq.com/404/) 可以获取代码，调用腾讯公益 404 页面的接口制作自己的公益 404 界面，代码如下：
```html
<script type="text/javascript" src="http://www.qq.com/404/search_children.js" charset="utf-8" homePageUrl="填写你的网站的 URL，建议设置为 '/'" homePageName="回到我的主页"></script>
```

注意需要修改 `homePageUrl` 参数，否则跳转不到你的主页了，完整代码如下：
```html
<!DOCTYPE HTML>
<html>
<head>
...
# 二. Github Pages 配置公益 404 页面 #

Github Pages 默认拦截非 `HTTPS` 请求(我也不知道是好事还是坏事)，而我们可以看到腾讯给的代码中所有的链接都是 `HTTP` 的(腾讯在 HTTPS 这条路上走的很慢啊)，所以就会导致一个很尴尬的问题，请求链接腾讯的 `JavaScript` 文件时，会被 `Github Pages` 拦截，下面是我的尝试过程

## 1. 尝试修改请求协议 ##

最直接的思路自然是尝试将 `http://` 修改为 `https://` 了，但是腾讯主站并没有开启 `HTTPS` 服务，所以接口自然找不到 `JavaScript` 文件

最终结果：失败

## 2. 尝试在 404 页面中直接调用 JavaScript 代码 ##

直接访问腾讯的接口文件(`http://www.qq.com/404/search_children.js`)，可以看到其实本质上这个文件是在调用 `qzone.qq.com/gy/404/data.js` 和 `qzone.qq.com/gy/404/page.js` ，这样事情就简单了，因为 QQ 空间是开启了 `HTTPS` 服务的(吐槽 QQ 开 `HTTPS` 也不全开)，所以直接添加这两条语句：
```html
<script src="https://qzone.qq.com/gy/404/data.js" charset="utf-8"></script>
<script src="https://qzone.qq.com/gy/404/page.js" charset="utf-8"></script>
```
直接添加在上一个 `JavaScript` 后面就行了，也就是：
```html
<!DOCTYPE HTML>
<html>
<head>
...
```
但是在使用过程中发现还是存在问题，不能引入 `CSS` 文件，所以只有纯 `HTML` 内容(在链接的 `JavaScript` 文件中引用了 `CSS` 文件，但是还是使用 `HTTP` )，特别尴尬，想来也不会有人看了，更别提什么公益效益了

最终结果：成功加载 `JavaScript` 文件，但是加载 `CSS` 文件时依旧遭到拦截
结果截图：
![加载不出 CSS](https://cloud.githubusercontent.com/assets/16161582/23393200/316b490e-fdbc-11e6-9a5e-69ae766829b4.png)
![加载 CSS 出现 304 错误](https://cloud.githubusercontent.com/assets/16161582/23393270/ac5495a8-fdbc-11e6-9351-a66876756528.png)

## 3. 将 `JavaScript` 文件本地化 ##

在本地建立对应的 `JavaScript` 文件，并在里面将引入的 `CSS` 样式链接更换为 `https://`即可，这样就能成功引入了，但是出现了一个意想不到的问题：Hexo 在生成 `Blog` 的时候，将我的 `JavaScript` 代码换了，原本的代码是：
```js
c += "<" + d + ">\n" + a[d] + "\n\n";
```
修改后的代码为：
...
感谢 Github 上的 `YuleYu ` 大神，提供了一种新的方式，直接更换 `API` 地址，这样就不需要本地化文件了，避免了本地化产生的未知错误，具体代码如下：
```html
<!DOCTYPE HTML>
<html>
<head>
...
<body>
    <script type="text/javascript" homePageUrl="https://he110.top" homePageName="返回主页" src="https://qzone.qq.com/gy/404/data.js"></script>
...
# 总结 #
`Github` 是个好东西，尤其是 `issue`，有时答案就在这里~